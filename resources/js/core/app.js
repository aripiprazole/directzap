...
(function (window, document, $) {
  ...
  $(window).on('load', function () {
    ...
    $('.scrollable-container').each(function () {
      const scrollable_container = new PerfectScrollbar($(this)[0], {
    ...
    $('a[data-action="reload"]').on('click', function () {
      const block_ele = $(this).closest('.card');
    ...
    if ($body.hasClass('boxed-layout')) {
      if ($body.hasClass('vertical-overlay-menu')) {
        ...
        const contentPosition = $('.app-content').position().left;
        const menuPositionAdjust = contentPosition - menuWidth;
    ...
    */
    function checkTextAreaMaxLength(textBox, e) {
      const maxLength = parseInt($(textBox).data('length')),
    ...
    */
    ...
    $('.content-overlay').on('click', function () {
      ...
      const searchInput = $('.search-input-close').closest('.search-input');
    ...
    const container = document.getElementsByClassName('main-menu-content');
  ...
  if (typeof Hammer !== 'undefined') {
    ...
    // Swipe menu gesture
    const swipeInElement = document.querySelector('.drag-target');
    let swipeInAction = 'panright',
    ...
    if ($(swipeInElement).length > 0) {
      const swipeInMenu = new Hammer(swipeInElement);
    ...
    setTimeout(function () {
      const swipeOutElement = document.querySelector('.main-menu');
      let swipeOutMenu;
    ...
    const swipeOutOverlayElement = document.querySelector('.sidenav-overlay');

    if ($(swipeOutOverlayElement).length > 0) {
      const swipeOutOverlayMenu = new Hammer(swipeOutOverlayElement);
  ...
  $('#sidebar-page-navigation').on('click', 'a.nav-link', function (e) {
    ...
    const scrollto = offset.top - 80; // minus fixed header height
    $('html, body').animate(
      {
        scrollTop: scrollto
      },
  ...
  if ($body.attr('data-framework') === 'laravel') {
    ...
    $('.dropdown-language .dropdown-item').on('click', function () {
      ...
      const selectedFlag = $this.find('.flag-icon').attr('class');
      $('#dropdown-flag .selected-language').text(selectedLang);
      $('#dropdown-flag .flag-icon').removeClass().addClass(selectedFlag);
  ...
  bookmarkStar.on('click', function (e) {
    ...
    const arrList = $('ul.nav.navbar-nav.bookmark-icons li');
    let $arrList = '',
      $activeItemClass = '';
    ...
    for (let i = 0; i < arrList.length; i++) {
      ...
      if ($(arrList[i].firstChild.firstChild).hasClass('feather')) {
        const classString = arrList[i].firstChild.firstChild.getAttribute('class');
  ...
  navLinkSearch.on('click', function () {
    const $this = $(this);
    const searchInput = $(this).parent('.nav-search').find('.search-input');
  ...
  if ($('.search-list-bookmark').length) {
    const searchListBookmark = new PerfectScrollbar('.search-list-bookmark', {
      wheelPropagation: false
    });
  }
  ...
  searchInputInputfield.on('keyup', function (e) {
    ...
    if (e.keyCode !== 38 && e.keyCode !== 40 && e.keyCode !== 13) {
      ...
      let //get values of input on keyup
        activeClass = '',
        bookmark = false;
  ...
  $(document).on('click', '.bookmark-input .search-list .bookmark-icon', function (e) {
    ...
    if ($(this).hasClass('text-warning')) {
      ...
      e.preventDefault();
      const $url = $(this).parent()[0].href,
        $name = $(this).parent()[0].innerText;
      let $listItem = '';
      const $listItemDropdown = '';
      let iconName = $(this).parent()[0].firstChild.firstChild.dataset.icon;
      if ($($(this).parent()[0].firstChild.firstChild).hasClass('feather')) {
        const classString = $(this).parent()[0].firstChild.firstChild.getAttribute('class');
        iconName = classString.split('feather-')[1].split(' ')[0];
      }
      $listItem =
  ...
  $(window).on('keydown', function (e) {
    ...
    if (e.keyCode === 13 && $('.search-list li.current_item').length > 0) {
      const selected_item = $('.search-list li.current_item a');
  ...
  const dataLayout = $html.attr('data-layout') ? $html.attr('data-layout') : 'light-layout';

  // Navbar Dark / Light Layout Toggle Switch
  $('.nav-link-style').on('click', function () {
    const currentLayout = getCurrentLayout();
    let switchToLayout = '';
    const prevLayout = localStorage.getItem(dataLayout + '-prev-skin', currentLayout);
  ...
  const currentLocalStorageLayout = localStorage.getItem(dataLayout + '-current-skin');
  ...
  function setLayout(currentLocalStorageLayout) {
    const navLinkStyle = $('.nav-link-style'),
